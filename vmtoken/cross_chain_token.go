...
func SpawnCrossChainToken(env vmmSchema.Env) (vm vmmSchema.Vm, err error) {
	...
	if feeRecipient == "" {
		feeRecipient = env.Meta.AccId // Default to owner
	}
	...
	if mintOwner == "" {
		mintOwner = env.Meta.AccId // Default to owner
	}
	...
	if burnProcessor == "" {
		burnProcessor = env.Meta.AccId // Default to owner
	}
	...
	}, env.Meta.AccId, mintOwner, burnFee, feeRecipient, burnProcessor)
...
func (v *CrossChainToken) Apply(from string, meta vmmSchema.Meta) (res vmmSchema.Result) {
	switch meta.Action {
	case "Info":
...
func (v *CrossChainToken) HandleInfo(from string) (res vmmSchema.Result) {
